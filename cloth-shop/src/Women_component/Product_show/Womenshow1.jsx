...
function Womenshow1() {
  ...
  const { womenData } = useProduct()
  const { id } = useParams();
  const [selectedColor, setSelectedColor] = useState([])
  const [selectedSize, setSelectedSize] = useState([])
  useEffect(() => {
    if (womenData && womenData.length > 0) {
      const selectedItem = womenData.find(item => item.id === parseInt(id));
  ...
  const addhandler = () => {
    const selectedProduct = womenData.find(item => item.id === parseInt(id));
